            ...
            function ast_inject(ast, code) {
                ...
                var ws = {
                    ...
                    "if": function (cond, t, e) {
                        ...
                        var xcode = code.substring(ce.endpos, ts.pos);
                        var tok = uglifyjs.parser.tokenizer(xcode);
                        
                        var token;
                        while ((token = tok()).type !== "eof") {
                            if (token.type === "punc" && token.value === ")") {
                                ifbl = [ { start: { pos: ce.endpos + token.pos, endpos: ce.endpos + token.endpos } } ];
                                break;
                            }
                        }
                            ...
                            return block(
                                ...
                                [
                                    ...
                                    MAP(body, function (branch) {
                                        var sbidx = startElement(branch).start.pos;
                                        var xcode = code.substring(eidx, sbidx);
                                        var leidx = eidx;
                                        eidx = endElement(branch).end.endpos;
