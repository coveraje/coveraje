...
(function () {
    ...
    function init(runtime, option) {
        ...
        var injector = (function () {
            ...
            function asBlock(stmt) {
                ...
                if (stmt != null && stmt.type === "BlockStatement") {
                    if (args.length > 1) {
                        stmt.body = spliceFlat(args.slice(1), stmt.body);
                    }
                    return stmt;
                }
                return {
            ...
            function ast_inject(ast, code, key) {
                ...
                function visit_retIfSimple(node) {
                    if (node != null) {
                        if (isSimple(node)) {
                ...
                var ws = {
                    ...
                    DoWhileStatement: function (node) {
                        ...
                        node.test = visit_retIfSimple(node.test);
                    },
                    ...
                    IfStatement: function (node) {
                        ...
                        // that's always there (beside "if" itself), use this to indicate how
                        // many times the consequent-part is hit
                    ...
                    LogicalExpression: function (node) {
                        // TODO: logical expressions should be tested separately (every combination or MC/DC)
                        node.left = visit_ret(node.left, node.left);
                        node.right = visit_ret(node.right, node.right);
                    },
                    ...
                    SwitchStatement: function (node) {
                        ...
                        node.discriminant = visit_retIfSimple(node.discriminant);
        ...
        function prepare(code) {
            ...
            if (option.beautify) {
                try {
                    code = esprima.generate(esprima.parse(code, {range: true, comment: true}));
                } catch (ex) {
                }
            } else {
                code = code.replace(/\r(?=\n)/g, "").replace(/\r/g, "\n");
