    ...
    function Coveraje(code, runner, options) {
        ...
        var injector = (function () {
                        ...
                        if (start) {
                            ...
                            if (pos != null) {
                                ...
                                if (eo != null && eo.end != null) {
                                    ...
                                    runtime.visit.register(key, pos, endpos);
                                    return ret;
                                }
                            }
                        }
            ...
            var addVisitAndReturnValue = visithelp(function (expr, key, pos, endpos) {
                return callRuntime("visit", "callWithRet", [ [ "string", key ], expr, [ "num", pos ], [ "num", endpos ] ]);
            });
            ...
            var callVisitFunc = visithelp(function (expr, key, pos, endpos) {
                return stat(callRuntime("visit", "call", [ [ "string", key ], [ "num", pos ], [ "num", endpos ] ]));
            });
            
            function callVisitFuncParams(func, key, args) {
                var argArr = args.map(function (arg) {
                    runtime.visit.register(key, arg.pos, arg.endpos);
                    return [ "object", [ [ "pos", [ "num", arg.pos ] ], [ "endpos", [ "num", arg.endpos ] ] ] ];
                });
                if (argArr.length === 0) return null;
                return stat(callRuntime("visit", "funcargs", [ [ "string", key], [ "name", "arguments" ], [ "array", argArr] ]));
            }
            
            var addVisit = visithelp(function (expr, key, pos, endpos) {
                var vc = stat(callRuntime("visit", "call", [ [ "string", key ], [ "num", pos ], [ "num", endpos ] ]));
            ...
            function branchhelp(r) {
                return function (expr, key, branch, keyword, isElse) {
                    ...
                    if (ks && ks.start) {
                        ...
                        runtime.branch.register(key, kpos, kend, pos, endpos, isElse);
            ...
            var addBranchSwitch = branchhelp(function (expr, key, keywordPos, keywordEndpos, branchPos, branchEndpos) {
                var bc = callRuntime("branch", "call", [ [ "string", key ], [ "num", keywordPos ], [ "num", keywordEndpos ], [ "num", branchPos ], [ "num", branchEndpos ] ]);
            ...
            var addBranchIf = branchhelp(function (expr, key, keywordPos, keywordEndpos, branchPos, branchEndpos) {
                var ret = stat(callRuntime("branch", "call", [ [ "string", key ], [ "num", keywordPos ], [ "num", keywordEndpos ], [ "num", branchPos ], [ "num", branchEndpos ] ]));
        ...
        function addCode(name, code) {
            ...
            //console.log(codes[name].codeToRun)
            //require("fs").writeFileSync(idx + ".test.js", codes[name].codeToRun, 'utf8');
            return codes[name];
        }
