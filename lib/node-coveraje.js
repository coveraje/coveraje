    ...
    function runInConsole(options, instance) {
        ...
        mr
            .onComplete(function (key, context) {
                var reportData = runtime.reportData();
                var data = {
                    visited: [],
                    branches: []
                };
                
                var vl, vlt, clt = 0, cl = 0, k;
                
                for (k in reportData.visited) {
                    if (isOwn(reportData.visited, k)) {
                        data.visited = data.visited.concat(reportData.visited[k]);
                    }
                }
                for (k in reportData.branches) {
                    if (isOwn(reportData.branches, k)) {
                        data.branches = data.branches.concat(reportData.branches[k]);
                    }
                }
