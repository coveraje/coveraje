/*
    ...
    This helper supports the following options (quite the same as mocha(1)):
        ...
        ignore-leaks:       boolean                     ignore flobal variable leaks, default false
    
    !!!!!!!!!!!
    - The options are likely to change with https://github.com/visionmedia/mocha/issues/265
    - mocha.opts and some options (like require, watch, growl, and debug) currently not supported
...
(function() {
    ...
    exports.run = function (file, event, option, requireSubst) {
        ...
        if (option.slow != null) {
            mocha.reporters.Base.slow = parseInt(option.slow);
        }
        if (option.timeout != null) {
            suite.timeout = parseInt(option.timeout);
        }
        suite.bail(!!option.bail);
        
        // load
        suite.emit('pre-require', global, file);
        suite.emit('require', requireSubst(file), file);
        ...
        if (option.globals != null && Array.isArray(option.globals)) {
            reporter.globals(option.globals);
        }
